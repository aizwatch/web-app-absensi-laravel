...
function renderFilterTable(data) {
  ...
  tbody.innerHTML = data.map(row => {
    ...
    const adminBtnsF = _authUser?.role === 'admin'
      ? `<span style="display:inline-flex;gap:3px;margin-left:4px">
           <button class="btn-icon" style="font-size:11px;padding:1px 4px" title="Inject absensi" onclick="openInjectModal('${escHtml(String(row.pin))}','${escHtml(row.tanggal)}','${escHtml(row.nama||'')}')">⚙️</button>
           <button class="btn-icon" style="font-size:11px;padding:1px 4px" title="Riwayat perubahan" onclick="openRowHistory('${escHtml(String(row.pin))}','${escHtml(row.tanggal)}','${escHtml(row.nama||'')}')">📋</button>
         </span>` : '';
    return `<tr>
...
function exportFilter() {
  if (!_lastFilterData || !_lastFilterData.length) {
    alert('Tampilkan data dulu sebelum export.');
    return;
  }

  const rows = _lastFilterData
    .filter(r => r.scan_masuk)
    .map(r => {
      const dt = (jam) => jam ? `${r.tanggal} ${jam}` : '';
      return [
        `Attendance: ${r.nama || r.pin}`,
        r.nama || String(r.pin),
        'Attendance',
        dt(r.scan_masuk),
        dt(r.scan_istirahat1),
        dt(r.scan_istirahat2),
        dt(r.scan_pulang)
      ];
    });
